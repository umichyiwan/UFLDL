...
z4 = softmaxTheta * a3;
exp_z4 = exp(z4);
a4 = bsxfun(@rdivide, exp_z4, sum(exp_z4));

% backward propagation
delta4 = -(groundTruth - a4); %here we take derivative with z4, which is theta*a3. The result is groundTruth - a4.
delta3 = (softmaxTheta' * delta4) .* sigmoidGrad(z3);
delta2 = (stack{2}.w' * delta3) .* sigmoidGrad(z2);

softmaxThetaGrad = (1. / M) * delta4 * a3'  + lambda * softmaxTheta;
stackgrad{2}.w = (1. / M) * delta3 * a2';
stackgrad{2}.b = (1. / M) * sum(delta3, 2);
stackgrad{1}.w = (1. / M) * delta2 * data';
stackgrad{1}.b = (1. / M) * sum(delta2, 2);

cost = -(1. / M) * sum(sum(groundTruth .* log(a4))) + (lambda / 2.) * sum(sum(softmaxTheta.^2));
